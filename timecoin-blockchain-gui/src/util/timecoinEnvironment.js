...
const PY_FOLDER = '../timecoin/daemon';
...
const getTimecoinVersion = () => {
  let version = null;
  const exePath = getExecutablePath('timecoin');
  console.log("exePath", exePath)
  // first see if we can get a timecoin exe in a standard location relative to where we are
  try {
    ...
    console.log("e1", e1)
    // that didn't work, let's try as if we're in the venv or timecoin is on the path
    try {
      version = child_process
