...
global.daemon_rpc_ws = `wss://${self_hostname}:56624`;
...
function loadConfig(version) {
  try {
    ...
    // 1) TIMECOIN_ROOT environment variable
    // 2) version passed in and determined by the `timecoin version` call

    // check if TIMECOIN_ROOT is set. it overrides everything else
    console.log("version", version)
    const config_root_dir =
      'TIMECOIN_ROOT' in process.env
        ? process.env.TIMECOIN_ROOT
        : path.join(os.homedir(), '.timecoin', version);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 56624); // jshint ignore:line
