...
import { AlertDialog } from '@timecoin/core';
import {
  Typography,
  Button,
  Box,
...
import { timecoin_to_mojo } from '../../../util/timecoin';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      timecoinper_input.value === '' ||
      Number(timecoinper_input.value) === 0 ||
      !Number(timecoinper_input.value) ||
      isNaN(Number(timecoinper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const timecoinper = timecoin_to_mojo(timecoinper_input.value);
    const timecoinper_value = Number.parseInt(Number(timecoinper));
    const userpubkey = userpubkey_input.value;
    const amount = timecoin_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = timecoin_to_mojo(fee_input.value);
