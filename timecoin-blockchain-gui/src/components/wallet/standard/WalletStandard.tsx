...
import {
  ...
  Fee,
  Form,
  TextField as TimecoinTextField,
  AlertDialog,
  CopyToClipboard,
  Flex,
  Card,
...
import { /* mojo_to_timecoin_string, */ timecoin_to_mojo } from '../../../util/timecoin';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_timecoin_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of timecoin in the blockchain at the current
      ...
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the amount of Timecoin that you can currently use to make
            transactions. It does not include pending farming rewards, pending
            incoming transactions, and Timecoin that you have just spent but is not
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      openDialog(
        <AlertDialog>
          <Trans>
            Error: Cannot send timecoin to coloured address. Please enter a timecoin
            address.
          </Trans>
        </AlertDialog>,
      );
    ...
    if (address.slice(0, 12) === 'timecoin_addr://') {
    ...
    const amountValue = Number.parseFloat(timecoin_to_mojo(amount));
    const feeValue = Number.parseFloat(timecoin_to_mojo(fee));
