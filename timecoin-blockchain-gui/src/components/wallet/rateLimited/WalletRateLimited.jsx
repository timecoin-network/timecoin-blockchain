...
import { AlertDialog, Card, Flex } from '@timecoin/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const timecoinper_input = ip_parsed.limit;
    ...
    const timecoinper_value = Number.parseInt(Number(timecoinper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (timecoin per interval):{' '}
                  {mojo_to_timecoin_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = timecoin_to_mojo(fee_input.value);
