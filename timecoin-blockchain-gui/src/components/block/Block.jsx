...
export default function Block() {
  ...
  const poolReward = mojo_to_timecoin(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_timecoin(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const timecoinFees = blockRecord.fees
    ? mojo_to_timecoin(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: timecoinFees ? `${timecoinFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Timecoin blockchain
            </Trans>
          </Back>
        }
        action={
