...
import timecoinEnvironment from '../util/timecoinEnvironment';
import timecoinConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!timecoinEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    timecoinConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
     ...
     // console.log('startUrl', startUrl);
      ...
      if (!timecoinConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !timecoinConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (timecoinConfig.manageDaemonLifetime()) {
        timecoinEnvironment.startTimecoinDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
		  {
            label: i18n._(/* i18n */ { id: 'Timecoin Blockchain Website' }),
            click: () => {
              openExternal(
                'https://timecoincrypto.net',
              );
            },
          },
		  {
            ...
            click: () => {
              openExternal(
                'https://www.timecoincrypto.net/faq',
              );
            },
          },
		  {
            label: i18n._(/* i18n */ { id: 'Timecoin Calculator' }),
            click: () => {
              openExternal(
          ...
          {
            label: i18n._(/* i18n */ { id: 'Timecoin Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Timecoin-Network/timecoin-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Timecoin-Network/timecoin-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Timecoin-Network/timecoin-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Timecoin-Network/timecoin-blockchain/issues',
              );
            },
          },
		  {
		  ...
		  {
            label: i18n._(/* i18n */ { id: 'Join Timecoin on Discord' }),
            click: () => {
              openExternal('https://discord.gg/wVAd75mJYR');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow Timecoin on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/TimecoinCryptoNet');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Timecoin Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Timecoin' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Timecoin Blockchain' }),
            click: () => {
              openAbout();
            },
          },
