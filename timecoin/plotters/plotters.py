...
from timecoin.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from timecoin.plotters.chiapos import get_chiapos_install_info, plot_timecoin
from timecoin.plotters.madmax import get_madmax_install_info, plot_madmax
from timecoin.plotters.install_plotter import install_plotter
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in TIMECOIN_ROOT.
    TIMECOIN_ROOT_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within TIMECOIN_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    build_parser(subparsers, root_path, timecoin_plotter, "chiapos", "Chiapos Plotter")
    ...
    if args.plotter == "chiapos":
        plot_timecoin(args, TIMECOIN_ROOT_path)
    if args.plotter == "madmax":
        plot_madmax(args, TIMECOIN_ROOT_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, TIMECOIN_ROOT_path, root_path)
