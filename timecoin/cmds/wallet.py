...
@wallet_cmd.command("send", short_help="Send timecoin to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-a", "--amount", help="How much timecoin to send, in TIMECOIN", type=str, required=True)
@click.option(
    "-m",
    "--fee",
    help="Set the fees for the transaction, in TIMECOIN",
    type=str,
    default="0",
    show_default=True,
    required=True,
)
@click.option("-t", "--address", help="Address to send the TIMECOIN", type=str, required=True)
@click.option(
...
@wallet_cmd.command("send_from", short_help="Transfer all timecoin away from a specific puzzle hash")
@click.option(
    "-p",
    "--rpc-port",
    help=(
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-s", "--source", help="Address to send the TIMECOIN from", type=str, required=True)
@click.option("-t", "--address", help="Address to send the TIMECOIN", type=str, required=True)
def send_from_cmd(
...
@click.option(
    "--launcher-hash",
    help="Set the launcher hash, you should get it from timecoin wallet",
    type=str,
    default=None,
)
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
