...
from timecoin.consensus.coinbase import create_puzzlehash_for_pk
from timecoin.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_timecoin_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from timecoin.util.bech32m import encode_puzzle_hash
from timecoin.util.config import (
    create_default_timecoin_config,
    initial_config_file,
    load_config,
    save_config,
...
from timecoin.util.ssl_check import (
...
from timecoin.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
from timecoin.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'timecoin keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "timecoin_target_address" not in config["farmer"]
    stop_searching_for_pool = "timecoin_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("timecoin_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("timecoin_target_address"):
                stop_searching_for_pool = True

    # Set the destinations, if necessary
    updated_target: bool = False
    if "timecoin_target_address" not in config["farmer"]:
        print(
            f"Setting the timecoin destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        config["farmer"]["timecoin_target_address"] = all_targets[0]
        updated_target = True
    elif config["farmer"]["timecoin_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['timecoin_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "timecoin_target_address" not in config["pool"]:
        print(f"Setting the timecoin destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        config["pool"]["timecoin_target_address"] = all_targets[0]
        updated_target = True
    elif config["pool"]["timecoin_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['timecoin_target_address']} with {all_targets[0]}"
        )
    if updated_target:
        print(
            f"To change the TIMECOIN destination addresses, edit the `gl_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    timecoin_ca_crt, timecoin_ca_key = get_timecoin_ca_crt_key()
    timecoin_ca_crt_path = ca_dir / "timecoin_ca.crt"
    timecoin_ca_key_path = ca_dir / "timecoin_ca.key"
    write_ssl_cert_and_key(timecoin_ca_crt_path, timecoin_ca_crt, timecoin_ca_key_path, timecoin_ca_key)
    ...
    timecoin_ca_crt, timecoin_ca_key = get_timecoin_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, timecoin_ca_crt, timecoin_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False, testnet: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if (
                timecoin_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet) == 0
        ...
        return timecoin_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet)


def timecoin_version_number() -> Tuple[str, str, str, str]:
...
def timecoin_minor_release_number():
    res = int(timecoin_version_number()[2])
    print(f"Install release number: {res}")
    return res


def timecoin_full_version_str() -> str:
    major, minor, patch, dev = timecoin_version_number()
...
def timecoin_init(
    ...
    """
    if os.environ.get("TIMECOIN_ROOT", None) is not None:
        print(
            f"warning, your TIMECOIN_ROOT is set to {os.environ['TIMECOIN_ROOT']}. "
            f"Please unset the environment variable and run timecoin init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Timecoin directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if TIMECOIN_ROOT is set, or if user has run timecoin init twice
        # before a new update.
        if testnet:
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'timecoin keys show --show-mnemonic-seed'")
