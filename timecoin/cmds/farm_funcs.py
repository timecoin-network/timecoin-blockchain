...
from timecoin.cmds.units import units
...
from timecoin.rpc.full_node_rpc_client import FullNodeRpcClient
from timecoin.rpc.wallet_rpc_client import WalletRpcClient
...
from timecoin.util.ints import uint16, uint64
from timecoin.util.misc import format_bytes, format_minutes
from timecoin.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total timecoin farmed: {amounts['farmed_amount'] / units['timecoin']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['timecoin']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['timecoin']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print(
                "For details on farmed rewards and fees you should run 'timecoin start wallet' and 'timecoin wallet show'"
            )
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'timecoin wallet show'")
    else:
        print("Note: log into your key using 'timecoin wallet show' to see rewards for each key")
