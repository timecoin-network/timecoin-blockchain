...
from timecoin.cmds.configure import configure_cmd
from timecoin.cmds.farm import farm_cmd
from timecoin.cmds.init import init_cmd
from timecoin.cmds.keys import keys_cmd
from timecoin.cmds.netspace import netspace_cmd
from timecoin.cmds.passphrase import passphrase_cmd
from timecoin.cmds.plotnft import plotnft_cmd
from timecoin.cmds.plots import plots_cmd
from timecoin.cmds.plotters import plotters_cmd
from timecoin.cmds.show import show_cmd
from timecoin.cmds.start import start_cmd
from timecoin.cmds.stop import stop_cmd
from timecoin.cmds.wallet import wallet_cmd
from timecoin.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
from timecoin.util.keychain import set_keys_root_path, supports_keyring_passphrase
from timecoin.util.ssl_check import check_ssl
...
@click.group(
    help=f"\n  Manage timecoin blockchain infrastructure ({DIST_NAME} {__version__})\n",
    epilog="Try 'timecoin start node', 'timecoin netspace -d 192', or 'timecoin show -s'",
...
def cli(
    ...
    if passphrase_file is not None:
        from timecoin.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
@cli.command("version", short_help="Show timecoin version")
def version_cmd() -> None:
    print(DIST_NAME, __version__)


@cli.command("run_daemon", short_help="Runs timecoin daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by timecoin start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio

    from timecoin.daemon.server import async_run_daemon
