...
ROOT = pathlib.Path(importlib.import_module("timecoin").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `timecoin` file name (which is the executable)
    and the `timecoin` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `timecoin` directory, since the names collide.
    ...
    for data in analysis.datas:
        if str(data[0]).startswith("timecoin/"):
            zipped.append(data)
        else:
            datas.append(data)
...
version_data = copy_metadata(get_distribution("timecoin-blockchain"))[0]
...
# TODO: collapse all these entry points into one `timecoin_exec` entrypoint that accepts the server as a parameter

entry_points = ["timecoin.cmds.timecoin"] + [f"timecoin.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    timecoin_mod = importlib.import_module("timecoin")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/timecoin/util/english.txt", "timecoin/util"))
datas.append((f"{ROOT}/timecoin/util/initial-config.yaml", "timecoin/util"))
datas.append((f"{ROOT}/timecoin/wallet/puzzles/*.hex", "timecoin/wallet/puzzles"))
datas.append((f"{ROOT}/timecoin/ssl/*", "timecoin/ssl"))
...
add_binary("timecoin", f"{ROOT}/timecoin/cmds/timecoin.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/timecoin/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/timecoin/server/start_{server}.py", COLLECT_ARGS)
