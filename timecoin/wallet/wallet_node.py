...
from timecoin.daemon.keychain_proxy import (
    KeychainProxy,
...
from timecoin.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from timecoin.protocols import farmer_protocol, wallet_protocol
from timecoin.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from timecoin.types.coin_spend import CoinSpend
...
from timecoin.util.byte_types import hexstr_to_bytes
from timecoin.util.check_fork_next_block import check_fork_next_block
from timecoin.util.errors import Err, ValidationError
from timecoin.util.ints import uint32, uint64, uint128
from timecoin.util.keychain import Keychain
from timecoin.util.lru_cache import LRUCache
from timecoin.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from timecoin.util.path import mkdir, path_from_root
from timecoin.util.profiler import profile_task
from timecoin.wallet.block_record import HeaderBlockRecord
from timecoin.wallet.derivation_record import DerivationRecord
from timecoin.wallet.settings.settings_objects import BackupInitialized
from timecoin.wallet.transaction_record import TransactionRecord
from timecoin.wallet.util.backup_utils import open_backup_file
from timecoin.wallet.util.wallet_types import WalletType
from timecoin.wallet.wallet_action import WalletAction
from timecoin.wallet.wallet_blockchain import ReceiveBlockResult
from timecoin.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    server: Optional[TimecoinServer]
    log: logging.Logger
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'timecoin keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSTimecoinConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def update_stakings(self, peer: WSTimecoinConnection, height: uint64, farmer_public_key: G1Element) -> None:
        "fetch staking"
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSTimecoinConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSTimecoinConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSTimecoinConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSTimecoinConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSTimecoinConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSTimecoinConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
