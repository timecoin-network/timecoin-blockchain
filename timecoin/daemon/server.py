...
from timecoin.cmds.init_funcs import check_keys, timecoin_init
from timecoin.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from timecoin.daemon.keychain_server import KeychainServer, keychain_commands
from timecoin.daemon.windows_signal import kill
from timecoin.plotters.plotters import get_available_plotters
from timecoin.plotting.util import add_plot_directory
from timecoin.server.server import ssl_context_for_root, ssl_context_for_server
from timecoin.ssl.create_ssl import get_mozilla_ca_crt
from timecoin.util.timecoin_logging import initialize_logging
from timecoin.util.config import load_config
from timecoin.util.json_util import dict_to_json_str
from timecoin.util.keychain import (
    Keychain,
...
from timecoin.util.service_groups import validate_service
from timecoin.util.setproctitle import setproctitle
from timecoin.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Timecoin.")
    quit()
...
service_plotter = "timecoin plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "timecoin": "timecoin",
        "timecoin": "timecoin",
        "timecoin_wallet": "start_wallet",
        "timecoin_full_node": "start_full_node",
        "timecoin_harvester": "start_harvester",
        "timecoin_farmer": "start_farmer",
        "timecoin_introducer": "start_introducer",
        "timecoin_timelord": "start_timelord",
        "timecoin_timelord_launcher": "timelord_launcher",
        "timecoin_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["timecoin", "plotters", plotter]
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("timecoin daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered TIMECOIN_ROOT
    os.environ["TIMECOIN_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with TIMECOIN_ROOT: {os.environ['TIMECOIN_ROOT']}")
    ...
    if service_command == "timecoin_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in timecoin_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    timecoin_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("timecoin_daemon")
