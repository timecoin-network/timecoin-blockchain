...
from timecoin.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from timecoin.full_node.full_node_api import FullNodeAPI
from timecoin.server.start_farmer import service_kwargs_for_farmer
from timecoin.server.start_full_node import service_kwargs_for_full_node
from timecoin.server.start_harvester import service_kwargs_for_harvester
from timecoin.server.start_introducer import service_kwargs_for_introducer
from timecoin.server.start_service import Service
from timecoin.server.start_timelord import service_kwargs_for_timelord
from timecoin.server.start_wallet import service_kwargs_for_wallet
from timecoin.simulator.start_simulator import service_kwargs_for_full_node_simulator
from timecoin.timelord.timelord_launcher import kill_processes, spawn_process
...
from timecoin.util.ints import uint16, uint32
from timecoin.util.keychain import bytes_to_mnemonic
...
async def setup_farmer(
    ...
    config["timecoin_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "timecoin")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["timecoin_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "timecoin")
