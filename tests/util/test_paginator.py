...
from chia.util.paginator import EmptyPageSizesError, InvalidPageSizeError, PageOutOfBoundsError, Paginator


def test_constructor() -> None:
    default_paginator: Paginator = Paginator([], 5)
    for i in range(0, default_paginator.page_sizes()[-1] + 1):
        if i in default_paginator.page_sizes():
...
def test_page_count() -> None:
    default_paginator: Paginator = Paginator([], 5)
    source = [i for i in range(0, default_paginator.page_sizes()[-1])]
    for page_size in default_paginator.page_sizes():
        for i in range(0, len(source)):
            paginator: Paginator = Paginator(source[0:i], page_size)
            assert paginator.page_count() == max(1, ceil(i / page_size))


@pytest.mark.parametrize("page_sizes", [(1,), (1, 2, 3)])
def test_page_sizes(page_sizes: Tuple[int, ...]) -> None:
    assert Paginator([], page_sizes[0], page_sizes).page_sizes() == page_sizes
    ...
    for i in range(0, paginator.page_sizes()[-1] + 1):
        if i not in paginator.page_sizes():
            with pytest.raises(InvalidPageSizeError):
                Paginator([0], i)
    with pytest.raises(EmptyPageSizesError):
        assert Paginator([], 1, ())
