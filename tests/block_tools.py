...
from timecoin.cmds.init_funcs import create_all_ssl, create_default_timecoin_config
...
from timecoin.consensus.coinbase import create_farmer_coin, create_pool_coin, create_puzzlehash_for_pk
from timecoin.consensus.condition_costs import ConditionCost
from timecoin.consensus.constants import ConsensusConstants
from timecoin.consensus.default_constants import DEFAULT_CONSTANTS
from timecoin.consensus.deficit import calculate_deficit
from timecoin.consensus.full_block_to_block_record import block_to_block_record
from timecoin.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from timecoin.consensus.pot_iterations import (
    calculate_ip_iters,
...
from timecoin.consensus.vdf_info_computation import get_signage_point_vdf_info
from timecoin.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
from timecoin.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from timecoin.full_node.generator import setup_generator_args
from timecoin.full_node.mempool_check_conditions import GENERATOR_MOD
from timecoin.full_node.signage_point import SignagePoint
from timecoin.plotting.create_plots import PlotKeys, create_plots
from timecoin.plotting.manager import PlotManager
from timecoin.plotting.util import PlotRefreshEvents, PlotRefreshResult, PlotsRefreshParameter, parse_plot_info
from timecoin.server.server import ssl_context_for_server
...
from timecoin.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
from timecoin.types.blockchain_format.pool_target import PoolTarget
from timecoin.types.blockchain_format.program import INFINITE_COST
from timecoin.types.blockchain_format.proof_of_space import ProofOfSpace
from timecoin.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
from timecoin.types.blockchain_format.sized_bytes import bytes32
from timecoin.types.blockchain_format.slots import (
...
from timecoin.types.generator_types import BlockGenerator, CompressorArg
...
from timecoin.util.block_cache import BlockCache
from timecoin.util.condition_tools import ConditionOpcode
from timecoin.util.config import load_config, save_config
from timecoin.util.errors import Err
from timecoin.util.hash import std_hash
from timecoin.util.ints import uint8, uint16, uint32, uint64, uint128
from timecoin.util.keychain import Keychain, bytes_to_mnemonic
from timecoin.util.merkle_set import MerkleSet
from timecoin.util.path import mkdir
from timecoin.util.prev_transaction_block import get_prev_transaction_block
from timecoin.util.vdf_prover import get_vdf_info_and_proof
from timecoin.wallet.derive_keys import (
...
class BlockTools:
    ...
    def __init__(
        ...
        create_default_timecoin_config(root_path)
    ...
    async def setup_keys(self):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="timecoin-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `timecoin generate keys`")
    ...
    async def new_plot(
        ...
        try:
            ...
            if pool_contract_puzzle_hash is None:
                ...
                pool_address = encode_puzzle_hash(pool_contract_puzzle_hash, "timecoin")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("TIMECOIN_ROOT", "~/.timecoin/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
